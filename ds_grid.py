...
# flow_grid = {
#     'n_coupling_layers': [4, 6, 8],
#     'flow_lr':           [1e-3, 1e-4],
#     'flow_wd':           [1e-2, 1e-3],
# }
# fusion_grid = {
#     'fusion_tau':       [0.5, 1.0, 1.5],
#     'fusion_lambda':    [0.1, 1.0],
#     'norm_pct':         [0.9, 0.95, 1.0],
#     'softmax_temp':     [1.0, 2.0],
# }
...
#                       list(flow_grid.keys()) +
#                       list(fusion_grid.keys()))
# all_params = []
# for fg in itertools.product(*flow_grid.values()):
#     for ug in itertools.product(*fusion_grid.values()):
#         d = dict(zip(flow_grid.keys(), fg))
#         d.update(zip(fusion_grid.keys(), ug))
#         all_params.append(ParamSet(**d))
...
all_params = [
    ParamSet(pe, nl, hd)
    for pe, nl, hd in itertools.product(pretrain_vals,
                                         nlayer_vals,
                                         hdim_vals)
]
...
for run, params in enumerate(all_params):
    ...
    # args.fusion_tau         = params.fusion_tau
    # args.fusion_lambda      = params.fusion_lambda
    # args.norm_pct           = params.norm_pct
    # args.softmax_temp       = params.softmax_temp
    # Get clean locations
    ...
    # # if this is the first run ever, write header
    # if run == 0 and not os.path.exists(summary_path):
    #     with open(summary_path, 'w') as sf:
    #         sf.write('run,'
    #                 'n_coupling_layers,flow_lr,flow_wd,'
    ...
    #     sf.write(f"{run},"
    #             f"{args.n_coupling_layers},{args.flow_lr},{args.flow_wd},"
    #             f"{args.fusion_tau},{args.fusion_lambda},{args.norm_pct},{args.softmax_temp},"
    #             f"{final_acc:.4f}\n")
    ...
    if run == 0 and not os.path.exists(summary_path):
        with open(summary_path, 'w') as sf:
            sf.write('run,pre_train_epochs,n_coupling_layers,coupling_hidden_dim,test_acc\n')
